...
The data used in this vignette is a lipidomics experiment from a cross-over nutritional intervention study. 10 subjects were given 2 dietary treatment, and blood samples were taken before and after each treatment.

Data can be downloaed from here.

```{r, eval = F}
data.raw = "https://raw.githubusercontent.com/zhuchcn/Metabase/master/data-raw/302870_Zhu_CSH-QTOF_lipidomics.xlsx"
standards = "https://raw.githubusercontent.com/zhuchcn/Metabase/master/data-raw/wcmc_lipidomics_standards.csv"
download.file(url = data.raw,  destfile = "")
download.file(url = standards, destfile = "wcmc_lipidomics_standards.csv")
```
...
```{r}
library(Metabase)
file = "302870_Zhu_CSH-QTOF_lipidomics.xlsx"
...
```{r, fig.height = 3, fig.width = 4.5, fig.align = "left"}
plot_hist_NA(mset)
```
...
```{r}
internal_standards = read.csv("wcmc_lipidomics_standards.csv")
...
The data now is cleaned up and is ready for any statistic analysis and visualization. You can see the quality of the test as below.

```{r, fig.width = 8, fig.height = 3, fig.align="left"}
