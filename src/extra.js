// The sidebar menu of mkdocs is a little crazy. Instead of create each separate
// sidebar menu for each top navbar item, it renders a whole sidebar menu, and 
// sets the list items that do not belong to the current top navbar item as 
// "visibility: hidden". So I have to change the format according to the router url
    ...
    if(window.location.pathname.search("post") === -1){
        ...
        for(i=0; i < items.length; i++){
            if(items[i].classList.contains("md-nav__item--active") !== true){
                ...
                const fas = items[i].querySelectorAll("i")
